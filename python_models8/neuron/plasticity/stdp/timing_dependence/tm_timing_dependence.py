...
from spynnaker.pyNN.models.neuron.synapse_dynamics.synapse_dynamics_stdp import (
    PlasticityTypes)
...
class TsodyksMarkramTimingDependence(AbstractTimingDependence):
    ...
    def __init__(
        ...
        self._delta_tau_inv = 1 / (tau_syn * tau_d)
    ...
    @property
    def delta_tau_inv(self):
        return self._delta_tau_inv
    ...
    def is_same_as(self, timing_dependence):
        ...
        return (self._synapse_type == timing_dependence.synapse_type and
                self._U == timing_dependence.U and
                self._tau_f == timing_dependence.tau_f and
                self._tau_d == timing_dependence.tau_d and
                self._tau_syn == timing_dependence.tau_syn and
                self._delta_tau_inv == timing_dependence.delta_tau_inv)
    ...
    def get_parameters_sdram_usage_in_bytes(self):
        ...
        # NOTE: usage for one set of parameters (6) and LUTs (3)

        return self.NUM_PARAMETERS * BYTES_PER_WORD + (BYTES_PER_WORD) * (len(self._tau_f_data) + len(self._tau_d_data) + len(self._tau_syn_data))
    ...
    def write_parameters(self, spec, weight_scales):
        ...
        fixed_point_synapse_type = float_to_fixed(self._synapse_type.value)
        fixed_point_tau_syn = float_to_fixed(self._tau_syn)
        fixed_point_U = float_to_fixed(self._U)
        fixed_point_tau_f = float_to_fixed(self._tau_f)
        fixed_point_tau_d = float_to_fixed(self._tau_d)
        fixed_point_delta_tau_inv = float_to_fixed(self._delta_tau_inv)
        ...
        # Write parameters
        spec.write_value(data=fixed_point_synapse_type, data_type=DataType.INT32)
    ...
    def get_parameter_names(self):
        return ['synapse_type', 'U', 'tau_f', 'tau_d', 'tau_syn']
